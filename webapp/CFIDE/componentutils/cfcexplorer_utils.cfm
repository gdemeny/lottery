 ...
 * convert to list/listfind method because we don't want to worry about delimiters
 *
	...
	if ( dir.exists() and not isShadowed ) {
		...
		for ( i = 1; isDefined("fileNames") and i lte ArrayLen(fileNames); i=i+1 ) {
			...
			if ( Find( '.', name ) eq 0 ) {
				...
				if ( file.isFile() ) {
					component = StructNew() ;
					component.name = packagePrefix & ListFirst( name, '.' ) ;
					component.path = Replace( file.getPath(), '\', '/', 'ALL' ) ;
					component.package = package ;
					component.cfcroot = cfcroot ;
